...
public class MicrophoneListener implements Runnable {
	...
	myThread.start();
	LogManager.e("decoded>>>>>>>>>    MicrophoneListener  <<<<<<<");
    }
...
//    		int buffSize = (Constants.kSamplesPerDuration) * 2 * 24;    		
    	    int buffSize =  (int) (16000 
    	    						* 2 // 2 seconds in the buffer
    	    						* 2); // recording in 16 bit
//    	    int buffSize = AudioRecord.getMinBufferSize(16000, 1, AudioFormat.ENCODING_PCM_16BIT);
    	    int biteSize = (Constants.kSamplesPerDuration);  // move 1/8 sec of data at a time
    	    ...
    	    //ByteArrayOutputStream out  = new ByteArrayOutputStream();
     	    ...
     	    int numBytesRead;
    	    arec.startRecording();
//    	    arec.read(data_16bit, 0, data_16bit.length);
    	    while(running)
    	    {
    		    ...
    		    //LogManager.e(Thread.currentThread().getName() + "> bytesRead = " + numBytesRead);
...
//     		    Thread.sleep(10);
    	    ...
    	    LogManager.e("decoded>>>>>>>>>    MicrophoneListener  error<<<<<<<");
//    	    try {
//				Thread.sleep(100);
