...
public class Encoder implements Constants {
    ...
    */
    ...
    public static byte[] appendCRC(byte[] input) {
    	...
    	output[input.length] = (byte) (crc8 & 0x0f);
    	output[input.length + 1] = (byte) (crc8 >> 4 & 0x0f);
    	output[input.length + 2] = 0x0e;
    	output[input.length + 3] = 0x0e;
    	return output;
    }
	...
	return results;
	 ...
	 double minVal = 0;
	 double maxVal = 0;
	 int peaks = (int)(input.length * magicScalingNumber);
	...
	double magicScalingNumber = 0.0001;
