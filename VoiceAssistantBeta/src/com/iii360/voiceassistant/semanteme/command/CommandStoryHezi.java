...
public class CommandStoryHezi extends AbstractVoiceCommand {
	private MediaUtil mediaUtil;
	private MediaInfoList mMediaList;
	private CommandInfo mCommandInfo;

	public CommandStoryHezi(BasicServiceUnion union, CommandInfo commandInfo) {
		super(union, commandInfo, COMMAND_NAME_PLAY_MEDIA, "播放故事");
...
//		}).start();
	...
	private void playMedia(int position) {
		getUnion().getBaseContext().setGlobalInteger(KeyList.GKEY_PLAY_TYPE, KeyList.GKEY_PLAY_TYPE_STORY);

		DlanMusicController mDlanMusicController = new DlanMusicController(mContext);
		android.util.Log.e("hefeng", "CommandPlayMedia is dlan play:"
				+ mDlanMusicController.isDlan());
		...
		getUnion().getMediaInterface().setPlayType(3);
